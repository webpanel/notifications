...
export var NotificationsMenu = function (props) {
    ...
    return (React.createElement(NoticeIcon, { count: (data && data.filter(function (x) { return !x.seen; }).length) || 0, loading: loading, onClear: onClear, onItemClick: onItemClick(collection, reload) }, _tabs.map(function (tab, i) {
        ...
        return (React.createElement(NoticeIcon.Tab, { key: tab.channel + "_" + i, tabKey: "" + i, list: notifications, count: notifications.filter(function (x) { return !x.seen; }).length, 
            // skeletonProps={{}}
            title: tab.title, showClear: false }));
    })));
};
